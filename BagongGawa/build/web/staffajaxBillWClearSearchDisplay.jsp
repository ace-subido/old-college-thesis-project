    ...
    for(int b = 0 ; b < vr.size(); b++)
    {
        ...
        if(!(z.equals(po)))
        {
            if(z.equalsIgnoreCase(q))
            {
                ...
                response.getWriter().write("<p>Mailing Address : "+vr.elementAt(b).address);
                response.getWriter().write("<br>Telephone Number : "+vr.elementAt(b).telenumber+"</p>");                
                //button
                response.getWriter().write("<p><a href='processStaffWBillClearSelectRes.jsp?num="+vr.elementAt(b).residentid+"'/>View Account</a></p>");
            }
        }
    }
%>