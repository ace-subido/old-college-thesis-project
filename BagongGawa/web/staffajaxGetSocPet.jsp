...
<%
    ...
    CachedRowSet ctv = sg.getStaffPendPet();
    ...
    else
    {
        ...
        response.getWriter().write("</select>&nbsp;&nbsp;<input type='button' value='Filter' onclick='staffFilterbyCat()'/></p>");                                           
        response.getWriter().write("<p><font color='red'> No Petition Requests!</font></p>");
        response.getWriter().write("</div>");
    }
        
    CachedRowSet ctv2 = sg.getStaffPetforApp();
    CachedRowSet crset2 = sg.getSocGCat();
    ...
    if(ctv2.size() >0)
    {
        ...
        while(crset2.next())
        {
            response.getWriter().write("<option value='"+crset2.getString("scatName")+"'> "+crset2.getString("scatName")+" </option>");                       
        }                                   
        ...
        while(ctv2.next())
        {
            ...
            response.getWriter().write("<h2>&nbsp;&nbsp;"+ctv2.getString("SGName")+"</h2><br>");
            response.getWriter().write("<h3>&nbsp;&nbsp;Requested by : "+ctv2.getString("SGRHNme")+"</h3>");
            response.getWriter().write("<h3>&nbsp;&nbsp;Date : "+petdate+"</h3>");
            response.getWriter().write("<h3>&nbsp;&nbsp;Category : "+ctv2.getString("SGCategory")+"</h3>");
            response.getWriter().write("<p>&nbsp;&nbsp;"+ctv2.getString("SGDetails")+"</p>");
            response.getWriter().write("<p>&nbsp;&nbsp;<a href='#'>View Details</a> | <a href='#'>Remove Petition</a></p>");
            response.getWriter().write("</div>");
        }
        response.getWriter().write("</div>");
    }  
